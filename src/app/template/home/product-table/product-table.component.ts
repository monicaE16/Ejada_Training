...
export class ProductTableComponent implements OnInit {
 ...
 onBuy(p:product)
 {
   ...
   {
    this.totalPrice+=p.price*(p.availabelQty);
    this.numberOfItems+=(p.availabelQty)
    p.availabelQty-=(p.availabelQty);
   }
   
   else if(p.availabelQty>=p.qty)
   {
   this.totalPrice+=p.price*p.qty;
   this.numberOfItems+=p.qty
 ...
 setCategory(id:number)
 {
  ...
  {
 this.products=this.allProducts.filter(product=>product.categoryId===id);
  }
 }
  ngOnInit(): void {}
